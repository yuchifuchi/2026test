...
開発リポジトリの並びではなく、「誰が何を見ればよいか」で番号を振り直す。
  ...
  01_マニュアル/                  利用者・担当者向け
  02_データベース_Access/         Access に貼り付ける VBA と初期データ
...
def build(out_root):
    ...
    for f in sorted(os.listdir(os.path.join(HERE, "dist"))):
        copy(os.path.join(HERE, "dist", f), os.path.join(d, "貼り付けるVBA", f))
    copytree(os.path.join(HERE, "data"), os.path.join(d, "マスタ初期データ"))

    order = """電話応対日報 集計システム ― Access への貼り付け手順

Access が入っているパソコンで、1 回だけ行う作業です。

1. Access を起動し「空のデータベース」を作ります。
   ファイル名は 日報集計_be.accdb にしてください。

2. Alt キーを押しながら F11 キーを押します。
   （Visual Basic の画面が開きます）

3. メニューの「挿入」→「標準モジュール」を選びます。

4. 「貼り付けるVBA」フォルダの中のファイルを、下の順番で
   1 つずつ開いて、中身を全部コピーして貼り付けます。
   貼り付けたら、左側の一覧でモジュール名を下の名前に変えてください。
   ...
   ※「ファイル」→「ファイルのインポート」で .bas を直接読み込んでも構いません。
     このフォルダの .bas は Shift_JIS で保存してあるので文字化けしません。

5. modSetup を開き、Setup_DBOnly という行にカーソルを置いて F5 キーを押します。

     ★ Setup_All ではなく Setup_DBOnly です。
       Setup_All は Access 単体で使う場合のもので、今回は不要な画面まで作ります。

6.「データベースを作成しました」と出たら完了です。

7. できた 日報集計_be.accdb を共有フォルダに置きます。

8. 過去の Excel データを取り込む場合は、この時点で
   メニューの「Excel から取込」を実行してください。
   （詳しくは 01_マニュアル の 05_設置手順 を参照）

--------------------------------------------------------------------
うまくいかないとき

・「ユーザー定義型は定義されていません」と出る
  → Visual Basic の画面で「ツール」→「参照設定」を開き、
     Microsoft Office xx.x Access database engine Object Library
     にチェックを入れてください。

・貼り付けたら文字化けした
  → このフォルダの .bas ではなく、別の場所のファイルを使っていませんか。
     ここの .bas は Shift_JIS で保存してあります。
"""
    with open(os.path.join(d, "貼り付ける順番.txt"), "wb") as f:
...
------------------------------------------------------------
 ...
 02_データベース_Access
     Access に貼り付ける VBA と、マスタの初期データです。
     「貼り付ける順番.txt」の手順どおりに進めてください。
     この作業は 1 回だけです。
...
<main>
  ...
  <div class="cards">
    <a class="card" href="02_データベース_Access/貼り付ける順番.txt">
      <div class="n">02</div><b>データベースを作る</b>
      <span>Access に VBA を貼り付けて実行します。1 回だけの作業です。</span></a>
